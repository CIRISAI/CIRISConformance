...
| **SCOPE_PRIVACY §9 — `record_id` cross-impl reproducibility** (RFC 8949 dCE CBOR, HMAC-SHA3-256, HKDF-SHA{2,3}-256 KAT vectors) | CCS | ✅ | [`test_400_scope_privacy_record_id.py`](../tests/test_400_scope_privacy_record_id.py); clean-room Python oracle for the verify v6.3.0 §2.2/§2.4/§3.4 derivations. See [`SCOPE_PRIVACY_CONFORMANCE.md`](SCOPE_PRIVACY_CONFORMANCE.md). |
| **SCOPE_PRIVACY §9 — forensic cold-state disk inspection** (persist v9.2.0 V088 `federation_scope_blobs` schema opacity) | CCS | ✅ | [`test_410_scope_privacy_forensic.py`](../tests/test_410_scope_privacy_forensic.py); substrate-side `put_scope_blob` FFI gap tracked. |
| **SCOPE_PRIVACY §9 — per-scope Poisson KS-test + λ_real ≤ λ_cover** | CCS | ✅ (simulator) | [`test_420_scope_privacy_poisson.py`](../tests/test_420_scope_privacy_poisson.py); FSD §3.1 first-principles simulator. Wheel-driven variant pending edge `EmissionScheduler` PyO3 export. |
| **SCOPE_PRIVACY §9 — 20-holder cluster-detection KS-test** (FSD §7.5 defense) | CCS | ✅ (simulator) | [`test_430_scope_privacy_cluster_ks.py`](../tests/test_430_scope_privacy_cluster_ks.py); n=20, K=6 viable; pinned positive + negative cluster-width self-checks. |
