...
The **[CIRIS Constitution](../reference/CIRIS_Constitution/) `CC 1.0-rc1`** — the
...
## Why this harness can't yet cover all of CC 1.0-rc1
...
| CC 6.1.5.2 storage-contention `StorageBudgetV1`/`CorpusWantV1` (CC 0.9) | CCS | ✅ | `test_530` — `build_storage_budget_v1`/`verify_storage_budget_v1`/`storage_budget_supersedes` (anti-rollback) + `build_corpus_want_v1`/`verify_corpus_want_v1`/`corpus_want_admits` (size-cap/CID) shipped in persist 12.5.0 (**CIRISPersist#356** closed) |
...
| Live transport delivery (4-node fabric) | CCS | ✅ | `test_340` — `init_edge_runtime(enable_transport=True)` returns (~0.002s) and a primed peer receives over live Reticulum (**CIRISPersist#320** closed; the deadlock/crash is fixed). Four routes (self/family/community/direct) each assert real delivery to the addressed recipient |
