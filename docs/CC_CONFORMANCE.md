...
| CC 3.2 single-owner — reject-2nd-distinct-owner **admission** gate | CCS | ✅ | `test_551` — persist 13.4.1 enforces it (**CIRISPersist#378** closed): a second distinct owner-binding on the same node is rejected at admission; combined with `owner_of_json` the CC 3.2 single-owner invariant is fully gated |
