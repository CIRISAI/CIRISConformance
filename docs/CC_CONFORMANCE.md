...
| CC 3.4.8 detection discriminator (`detection:*` → `lenscore_detector`) | CCP/CCS | ✅ | `test_550` — an agent key on the enumerated `detection:*` leaves is refused (`federation_reserved_prefix_emitter_mismatch`); a `lenscore_detector`-in-`identity_type` key is admitted; `truth_grounding:detection:*` cross-attestations ride free. xfail: a *novel* `detection:{newkind}:*` isn't covered by a prefix-wildcard → **CIRISPersist#379** |
| CC 3.2 single-owner invariant (`owner_of` ≤ 1; reject 2nd distinct owner) | CCS | ⏳ | `test_551` xfail(strict) — the CC 2.4.1.2 owner-binding purpose + a single-owner admission gate + a purpose-filtered `owner_of` are not on the persist FFI (readers conflate purposes → cardinality 2). `owner_of` shipped at the *server* layer (CIRISServer#162), not the Engine → **CIRISPersist#378** |
