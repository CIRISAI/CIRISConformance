...
# The `federation` fixture above shares a SUBSTRATE (one sqlite file) so
# peers see each other's directory rows — enough for directory-level
# scenarios, but NOT a wire round-trip: no envelope crosses a transport.
# This fixture is the real thing — two separate edge PROCESSES with
# `enable_transport=True`, B listening on a fixed TCP port, A bootstrapping
# to it — and drives `send_inline_text(A→B)` against B's registered handler.
#
# Cold 2-node delivery needs each side to ROOT the other's peer mapping
# (key_id → reticulum dest-hash + transport ed25519 pubkey). Per edge's own
# loopback test (`tests/reticulum_loopback.rs`), v7.0.0 explicit-hash
# discovery is OUT-OF-BAND — edge primes the pair via the Rust-only
# `inject_rooted_peer_for_test`. The published wheel has no Python
# equivalent yet (CIRISEdge#214); this fixture calls `edge.prime_peer(...)`
# when present and otherwise reports the unreachable outcome, so the
# round-trip test xfails on #214 and flips green the moment that surface
# ships. The fixture infra (process orchestration, identity exchange,
# delivery polling) is ready now.
...
@pytest.fixture
def two_node_transport(tmp_path):
    """Bring up two real edge processes (B listens, A dials) and round-trip a send.

    Returns a dict with the outcome of A→B inline-text delivery over a live
    transport: `{"delivered": bool, "send_outcome": str, "b_received": bool,
    "primed": bool, ...}`. Both nodes enable transport, share one on-disk
    federation directory, and exchange their transport identities
    (`reticulum_dest_hash_hex` + ed25519 pubkey) through small ready-files so
    each can `prime_peer` the other when that surface exists (CIRISEdge#214).
    """
    import time

    db_file = tmp_path / "fed_transport.db"
    db_file.touch()
    db_url = f"sqlite:///{db_file}"
    port_b = _free_tcp_port()
    ready_b = tmp_path / "B.ready.json"
    ready_a = tmp_path / "A.ready.json"

    node_b = _two_node_b_script(db_url, str(port_b), str(ready_b), str(ready_a))
    node_a = _two_node_a_script(db_url, str(port_b), str(ready_a), str(ready_b))

    b_proc = subprocess.Popen([sys.executable, "-c", textwrap.dedent(node_b)],
                              stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    # Wait for B to publish its identity, then run A to completion.
    deadline = time.time() + 30
    while time.time() < deadline and not ready_b.exists():
        if b_proc.poll() is not None:
            out, err = b_proc.communicate()
            pytest.fail(f"node B exited before ready (rc={b_proc.returncode}):\n{err[-1500:]}")
        time.sleep(0.2)
    if not ready_b.exists():
        b_proc.kill()
        pytest.fail("node B never became ready within 30s")

    a_result = run_python_script(node_a, timeout=60)
    try:
        b_out, b_err = b_proc.communicate(timeout=40)
    except subprocess.TimeoutExpired:
        b_proc.kill()
        b_out, b_err = b_proc.communicate()

    payload = a_result.parsed_stdout()
    # B prints its final JSON (received?) on the last stdout line.
    try:
        b_payload = json.loads((b_out or "").strip().splitlines()[-1])
    except (ValueError, IndexError):
        b_payload = {"_unparsed": (b_out or "")[-500:], "_stderr": (b_err or "")[-500:]}
    payload["b_received"] = bool(b_payload.get("received"))
    payload["b_detail"] = b_payload
    return payload


def _two_node_b_script(db_url, port_b, ready_b, ready_a):
    head = (f"DB_URL = {db_url!r}\nPORT_B = {port_b!r}\n"
            f"READY_B = {ready_b!r}\nREADY_A = {ready_a!r}\n")
    return head + r'''
...
_idp = os.path.join(_d, "b.id"); open(_idp, "wb").write(b"\x00" * 64)
cp.reset_engine()
k = "nodeb-" + secrets.token_hex(8)
engine = cp.Engine(DB_URL, k, local_key_id=k, local_key_path=_s)
kid = engine.register_self_federation_key("agent", "node-B", None, None, None)
edge = init_edge_runtime(engine, _idp, listen_addr="127.0.0.1:" + PORT_B,
                         enable_transport=True, hybrid_policy="ed25519_fallback",
                         agent_occurrence_key_id=kid)
got = []
edge.register_inline_text_handler(lambda *a: got.append(a))
# Publish B's identity for A to root against.
ident = {"kid": kid,
         "dest_hash": edge.reticulum_dest_hash_hex(),
         "transport": edge.transport_identity_pubkeys()}
open(READY_B, "w").write(json.dumps(ident))
# Wait for A's identity, then prime A as a peer (if the surface exists).
primed = False
for _ in range(150):
    if os.path.exists(READY_A):
        break
    time.sleep(0.2)
if os.path.exists(READY_A):
    a = json.loads(open(READY_A).read())
    if hasattr(edge, "prime_peer"):
        try:
            edge.prime_peer(a["kid"], a["dest_hash"],
                            a["transport"]["ed25519_pub_base64"])
            primed = True
        except Exception as exc:
            primed = "error: " + str(exc)[:80]
# Wait for delivery.
deadline = time.time() + 25
while time.time() < deadline and not got:
    time.sleep(0.2)
print(json.dumps({"received": bool(got), "primed": primed,
                  "sample": str(got[:1])[:120]}))
sys.stdout.flush(); os._exit(0)
'''


def _two_node_a_script(db_url, port_b, ready_a, ready_b):
    head = (f"DB_URL = {db_url!r}\nPORT_B = {port_b!r}\n"
            f"READY_A = {ready_a!r}\nREADY_B = {ready_b!r}\n")
    return head + r'''
import os, json, sys, time, tempfile, secrets
import ciris_persist as cp
from ciris_edge.ciris_edge import init_edge_runtime
b = json.loads(open(READY_B).read())
_d = tempfile.mkdtemp()
_s = os.path.join(_d, "s"); open(_s, "wb").write(secrets.token_bytes(32))
_idp = os.path.join(_d, "a.id"); open(_idp, "wb").write(b"\x00" * 64)
cp.reset_engine()
k = "nodea-" + secrets.token_hex(8)
engine = cp.Engine(DB_URL, k, local_key_id=k, local_key_path=_s)
kid = engine.register_self_federation_key("agent", "node-A", None, None, None)
edge = init_edge_runtime(engine, _idp, listen_addr="127.0.0.1:0",
                         bootstrap_peers=["127.0.0.1:" + PORT_B],
                         enable_transport=True, hybrid_policy="ed25519_fallback",
                         agent_occurrence_key_id=kid)
# Publish A's identity so B can root it too.
open(READY_A, "w").write(json.dumps({
    "kid": kid, "dest_hash": edge.reticulum_dest_hash_hex(),
    "transport": edge.transport_identity_pubkeys()}))
report = {"primed": False}
if hasattr(edge, "prime_peer"):
    try:
        edge.prime_peer(b["kid"], b["dest_hash"],
                        b["transport"]["ed25519_pub_base64"])
        report["primed"] = True
    except Exception as exc:
        report["primed"] = "error: " + str(exc)[:80]
report["prime_peer_available"] = hasattr(edge, "prime_peer")
# Let the link settle, then attempt delivery to B with retries.
outcome = None
for _ in range(15):
    time.sleep(2)
    try:
        edge.send_inline_text(b["kid"], "two-node-roundtrip-body")
        outcome = "sent"; break
    except Exception as exc:
        outcome = str(exc)[:90]
report["send_outcome"] = outcome
report["delivered"] = outcome == "sent"
report["stage"] = "done"
print(json.dumps(report)); sys.stdout.flush(); os._exit(0)
'''
